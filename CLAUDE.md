...
1. **Root cause first** — one short paragraph naming the single actual defect, with the
   evidence (the wire bytes, the error column, the type mismatch). No preamble.
2. **Numbered list of every remaining defect** that will bite after the root cause is
...
3. **Stop there.** No code sketch, no signature, no snippet — describe the correct
